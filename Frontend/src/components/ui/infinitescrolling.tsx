...
export default function InfiniteScroll({
  ...
  useEffect(() => {
    ...
    // Store current positions to prevent reset on re-render
    const currentPositions = divItems.map(item => {
    ...
    divItems.forEach((child, i) => {
      const y = currentPositions[i] || i * totalItemHeight;
      gsap.set(child, { y });
    });
  ...
  return (
    ...
    >
      {/* Gradient Overlays */}
      <div className="absolute top-0 left-0 w-full h-1/4 bg-gradient-to-b from-background to-transparent z-10 pointer-events-none"></div>
      <div className="absolute bottom-0 left-0 w-full h-1/4 bg-gradient-to-t from-background to-transparent z-10 pointer-events-none"></div>
      ...
      >
        {items.map((item, i) => (
          <div
            className="flex items-center justify-center p-4 text-xl font-semibold text-center border border-border/50 rounded-[15px] select-none box-border relative bg-card/50 backdrop-blur-sm shadow-[0_0_15px_rgba(0,0,0,0.1)]"
            key={i}
            style={{
